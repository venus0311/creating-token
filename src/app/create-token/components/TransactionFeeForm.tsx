...
interface TransactionFeeFormProps {
  onChange: (values: { feeType?: string; taxPercentage?: string }) => void;
}

const TransactionFeeForm: React.FC<TransactionFeeFormProps> = ({
  onChange,
}) => {
    ...
    <div className="pt-6 sm:rounded-xl sm:border sm:border-background-300 sm:p-6 space-y-6">
      <div className="space-y-4">
        <div className="space-y-2">
          <h3 className="block text-sm/[1.5] font-medium text-text-500">
            Transaction Fee
          </h3>
          <p className="block text-text-300 text-sm/[1.5]">
            A fee is collected in your token and sent to treasury.
          </p>
        </div>
        <div className="space-y-2">
          <RadioGroup
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            <input
              ...
              id="tax-percentage"
            />
            <p className="block text-sm/[1.5] absolute right-4 top-1/2 -translate-y-1/2 font-medium text-text-500">
              %
            </p>
          </div>
        </div>
